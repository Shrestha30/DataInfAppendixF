...
def create_dataloaders(model_name_or_path="roberta-large",
    ...
    if given_noisy_datasets is None:
        noisy_datasets, noise_index=load_noisy_dataset_by_task(task=task, noise_ratio=noise_ratio)
    else:
        noisy_datasets = given_noisy_datasets
        noise_index = None
    
    if select_idx is not None:
        noisy_datasets['train'] = noisy_datasets['train'].select(select_idx)
        
    # print(noisy_datasets)
    # print('----------')
    # noisy_datasets['train'] = noisy_datasets['train'].select([2,7,8])
    # print(noisy_datasets)
    # print('---------')
    # for example in noisy_datasets['train']:
    #     print(example)
    ...
    return train_dataloader, eval_dataloader, noise_index, tokenized_datasets, collate_fn, noisy_datasets
